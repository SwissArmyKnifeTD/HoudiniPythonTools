...
def create_convex_hull(geo, points, normalize_parm = True, flip_normals_parm = True, simplify_parm = False, level_details = 0):
    """"
    Creates a convex hull based on the geo and points
    Args:
        geo = python node.geometry()
        points = the geometry points to be used
        normalize_parm, flip_normals_parm, simplify_parm, level_details = UI parms
    """
    ...
    try:
        ...
        # Simplify the points
        if simplify_parm and level_details > 0:
            # Create a grid for clustering
            grid_size = level_details
            # Round position to the grid and remove duplicate points
        ...
        # Create new points for the convex hull geo
        geo.clear()
        hull_points = []
        ...
        # Create polygons for each face with consistent orientation
        for face in hull.simplices:
            ...
            if len(face_points) >= 3 and normalize_parm:
                ...
                # Need to check if the normal is poiting outward from the centroid
                center_to_face =  face_points[0].position() - centroid_vector
                dot_product = normal.dot(center_to_face)
                
                # Deterine if we have to flip the face
        ...
        hou.ui.displayMessage("Scipy and or numpy modules not found", severity = hou.severityType.Error)
